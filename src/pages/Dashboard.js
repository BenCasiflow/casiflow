...
import { LayoutDashboard, Building2, User, Scale, LogOut, Bell, Download, Search, Star, TrendingUp, TrendingDown, MessageSquare } from 'lucide-react';
...
function Dashboard({ user, onLogout, userSettings, onUpdateSettings }) {
  ...
  const getNotificationMessage = (percent, type) => {
    if (percent >= 100) return type === 'deposit'
      ? `You have reached your monthly deposit limit. Any further deposits this month will exceed the limit you set for yourself.`
      : `You have reached your monthly net loss limit. Any further deposits this month will exceed the limit you set for yourself.`;
    if (percent >= 80) return type === 'deposit'
      ? `Almost at your limit — 80% of your monthly deposit budget has been spent across your casinos.`
      : `Almost at your limit — 80% of your monthly net loss budget has been spent across your casinos.`;
    if (percent >= 50) return type === 'deposit'
      ? `50% of your monthly deposit limit used. Still well within your limit — just a friendly heads up.`
      : `50% of your monthly net loss limit used. Still well within your limit — just a friendly heads up.`;
    return null;
  };
      ...
      <div style={styles.mainContent}>
        <div style={styles.topBar}>
          <div style={styles.topBarLeft}>
            ...
            <span style={styles.jurisdiction}>{user.jurisdiction || 'Global'}</span>
          </div>
        ...
        {activeNotification && showNotification && (
          <div style={{ ...styles.notificationBanner, backgroundColor: netLossLimitPercent >= 100 || depositLimitPercent >= 100 ? '#fef2f2' : '#fffbeb' }}>
            <span style={{ fontSize: '14px', color: netLossLimitPercent >= 100 || depositLimitPercent >= 100 ? '#dc2626' : '#92400e' }}>
        ...
        {casinos.length === 0 ? (
          <div style={styles.emptyDashboard}>
            <div style={styles.emptyDashboardContent}>
              ...
              <p style={styles.emptyDashboardText}>You have not added any casinos yet. Add your first casino to start tracking your spending and see your dashboard come to life.</p>
          ...
          <>
            <div style={styles.chartsRow}>
              <div style={styles.chartCard}>
                <h3 style={styles.sectionTitle}>Deposits vs Withdrawals by Casino</h3>
              ...
              <div style={styles.chartCard}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={trendData}>
                    ...
                    <Legend />
                    <Line type="monotone" dataKey="deposits" stroke="#0ea5e9" strokeWidth={2} dot={{ r: 4 }} name="Deposits" />
                    <Line type="monotone" dataKey="withdrawals" stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} name="Withdrawals" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            {allGameStats.length > 0 && (
              <div style={styles.gameStatsCard}>
        ...
        <div style={styles.casinoSection}>
            ...
            filteredCasinos.map(casino => {
              ...
              return (
                <div key={casino.id} style={styles.casinoCard}>
                  ...
                  <div style={styles.casinoStats}>
                    <span>Deposited: {symbol}{casino.deposits}</span>
                    <span>Withdrawn: {symbol}{casino.withdrawals}</span>
                    <span>Bonus: {symbol}{casino.bonuses}</span>
                    <span>Sessions: {casino.sessions}</span>
                    <span style={{ color: isWinning ? '#16a34a' : '#dc2626', fontWeight: '600' }}>Net Result: {isWinning ? '+' : '-'}{symbol}{Math.abs(casinoNetResult)}</span>
                  </div>
...
const styles = {
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 20px', color: 'rgba(255,255,255,0.6)', textDecoration: 'none', fontSize: '14px', fontWeight: '500', borderLeft: '3px solid transparent', transition: 'all 0.15s ease' },
  ...
  notificationBanner: { padding: '12px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0' },
  notificationClose: { backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#64748b' },
  ...
  heroLeft: {},
  heroLabel: { color: 'rgba(255,255,255,0.6)', fontSize: '13px', margin: '0 0 8px 0', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: '500' },
  heroAmount: { fontSize: '48px', fontWeight: '800', margin: '0 0 8px 0', letterSpacing: '-2px', lineHeight: 1 },
  ...
  heroStatLabel: { color: 'rgba(255,255,255,0.5)', fontSize: '10px', margin: '0 0 6px 0', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: '600' },
  heroStatValue: { color: 'white', fontSize: '15px', fontWeight: '700', margin: 0 },
  timeFilterBar: { display: 'flex', gap: '6px', padding: '14px 28px', backgroundColor: 'white', borderBottom: '1px solid #e2e8f0', flexWrap: 'wrap' },
  filterBtn: { padding: '5px 12px', borderRadius: '20px', border: '1px solid #e2e8f0', backgroundColor: 'white', color: '#64748b', fontSize: '12px', cursor: 'pointer', fontWeight: '500' },
  filterBtnActive: { backgroundColor: '#0ea5e9', color: 'white', border: '1px solid #0ea5e9' },
  statsRow: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', padding: '24px 28px 0 28px' },
  statCard: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderLeft: '4px solid #e2e8f0' },
  statLabel: { color: '#94a3b8', fontSize: '11px', margin: '0 0 10px 0', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: '600' },
  statValue: { color: '#0f172a', fontSize: '28px', fontWeight: '800', margin: '0 0 6px 0', letterSpacing: '-0.5px' },
  statSub: { color: '#94a3b8', fontSize: '12px', margin: 0 },
  limitsRow: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '16px 28px 0 28px' },
  limitCard: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  limitRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' },
  limitLabel: { color: '#374151', fontWeight: '600', margin: 0, fontSize: '14px' },
  limitInputRow: { display: 'flex', alignItems: 'center', gap: '8px' },
  limitInput: { width: '100px', padding: '6px 10px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '14px' },
  limitCurrency: { color: '#64748b', fontSize: '14px' },
  progressBar: { height: '10px', backgroundColor: '#f1f5f9', borderRadius: '5px', overflow: 'hidden', marginBottom: '8px' },
  progressFill: { height: '100%', borderRadius: '5px', transition: 'width 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: '8px' },
  progressLabel: { color: 'white', fontSize: '10px', fontWeight: '700' },
  limitText: { color: '#94a3b8', fontSize: '12px', margin: 0 },
  emptyDashboard: { margin: '24px 28px 0 28px', backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  emptyDashboardContent: { padding: '64px 40px', textAlign: 'center' },
  emptyIconWrapper: { width: '80px', height: '80px', backgroundColor: '#f0f9ff', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto' },
  emptyDashboardTitle: { color: '#0f172a', fontSize: '22px', fontWeight: '800', margin: '0 0 12px 0' },
  emptyDashboardText: { color: '#64748b', fontSize: '15px', lineHeight: '1.7', margin: '0 0 28px 0', maxWidth: '420px', marginLeft: 'auto', marginRight: 'auto' },
  emptyDashboardBtn: { display: 'inline-block', padding: '12px 28px', background: 'linear-gradient(135deg, #0ea5e9, #0369a1)', color: 'white', borderRadius: '10px', textDecoration: 'none', fontSize: '15px', fontWeight: '700', boxShadow: '0 4px 12px rgba(14,165,233,0.3)' },
  chartsRow: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '16px 28px 0 28px' },
  chartCard: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  sectionTitle: { color: '#0f172a', fontSize: '15px', fontWeight: '700', margin: '0 0 16px 0' },
  chartLegend: { display: 'flex', gap: '20px', marginTop: '12px', justifyContent: 'center' },
  legendItem: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#64748b' },
  legendDot: { width: '10px', height: '10px', borderRadius: '50%', display: 'inline-block' },
  tooltip: { backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px 16px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' },
  tooltipTitle: { color: '#1e293b', fontWeight: '700', fontSize: '14px', margin: '0 0 8px 0' },
  tooltipRow: { color: '#64748b', fontSize: '13px', margin: '4px 0' },
  gameStatsCard: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', margin: '16px 28px 0 28px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  gameStatsRow: { display: 'flex', flexDirection: 'column', gap: '12px' },
  gameStatItem: { display: 'flex', alignItems: 'center', gap: '12px' },
  gameStatName: { color: '#374151', fontSize: '13px', fontWeight: '600', margin: 0, width: '120px', flexShrink: 0 },
  gameStatAmount: { color: '#0f172a', fontSize: '13px', fontWeight: '700', margin: 0, width: '80px', flexShrink: 0 },
  ...
  recommendedCard: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', margin: '16px 28px 0 28px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '2px dashed #e2e8f0' },
  recommendedHeader: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' },
  ...
  casinoSection: { backgroundColor: 'white', borderRadius: '12px', padding: '20px', margin: '16px 28px 0 28px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  casinoListHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' },
  addBtn: { backgroundColor: '#0ea5e9', color: 'white', padding: '8px 16px', borderRadius: '8px', textDecoration: 'none', fontSize: '13px', fontWeight: '600' },
  ...
  emptyTitle: { color: '#1e293b', fontSize: '16px', fontWeight: '600', margin: '0 0 8px 0' },
  emptyText: { color: '#94a3b8', fontSize: '14px', margin: '0 0 16px 0' },
  emptyAddBtn: { display: 'inline-block', padding: '8px 20px', backgroundColor: '#0ea5e9', color: 'white', borderRadius: '8px', textDecoration: 'none', fontSize: '13px', fontWeight: '600' },
  casinoCard: { border: '1px solid #e2e8f0', borderRadius: '12px', padding: '20px', marginBottom: '12px', backgroundColor: '#fafafa' },
  casinoHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' },
  casinoNameRow: { display: 'flex', alignItems: 'center', gap: '12px' },
  casinoAvatar: { width: '44px', height: '44px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '18px', fontWeight: '800', flexShrink: 0 },
  casinoNameLine: { display: 'flex', alignItems: 'center', gap: '8px' },
  ...
  lastActivity: { color: '#94a3b8', fontSize: '12px', display: 'block', marginTop: '2px' },
  casinoHeaderRight: { display: 'flex', alignItems: 'center', gap: '12px' },
  starRating: { display: 'flex', gap: '2px' },
  streakBadge: { display: 'flex', alignItems: 'center', gap: '4px', backgroundColor: '#f1f5f9', color: '#64748b', fontSize: '12px', padding: '4px 10px', borderRadius: '20px', fontWeight: '500' },
  casinoBarRow: { marginBottom: '10px' },
  barLabelRow: { display: 'flex', justifyContent: 'space-between', marginBottom: '4px' },
  barLabel: { color: '#94a3b8', fontSize: '11px' },
  casinoMiniBar: { height: '6px', backgroundColor: '#f1f5f9', borderRadius: '3px', overflow: 'hidden', marginBottom: '4px' },
  casinoBarFill: { height: '100%', borderRadius: '3px', transition: 'width 0.3s ease' },
  casinoStats: { display: 'flex', gap: '16px', color: '#64748b', fontSize: '13px', marginBottom: '12px', flexWrap: 'wrap' },
  balanceRow: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' },
  balanceLabel: { color: '#374151', fontSize: '13px', fontWeight: '600', whiteSpace: 'nowrap' },
  balanceInput: { width: '120px', padding: '6px 10px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', backgroundColor: 'white' },
  ...
  casinoGameStats: { display: 'flex', gap: '8px', flexWrap: 'wrap' },
  gameTag: { backgroundColor: '#f0f9ff', color: '#0369a1', fontSize: '12px', padding: '4px 10px', borderRadius: '20px', fontWeight: '500' },
